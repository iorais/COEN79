...
namespace coen79_lab9
{
    ...
    // Library facilities used: cstdlib
    {
        ...
        {
            // copy current node into new_root
            binary_tree_node<Item> *new_root = new binary_tree_node<Item>(root_ptr->data());

            // copy left subtree into left child of new_root
            new_root->left() = tree_copy(root_ptr->left());

            // copy right subtree into right child of new_root
            new_root->right() = tree_copy(root_ptr->right());
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr == NULL)
            ...
            // nodes in left subtree + nodes in right subtree + 1 for current node
