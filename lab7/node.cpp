...
namespace coen79_lab7
{
    ...
    void list_tail_insert(node*& tail, const std::string &newName, const float &newPrice) {
        node* newTail = new node(newName, newPrice); // create new tail
        tail->setLink(newTail); // connect newTail to tail
    ...
    void list_clear(node*& head) {
        if(!head) // base case
            return;
        node* next = head->getLink(); // node after head
        list_clear(next); // recursively traverse
        delete head;// delete in post-order traversal
    }
    ...
    void list_head_remove(node*& head) {
        node* newHead = head->getLink(); // set newHead to second item in list
        delete head; // clear memory of head
